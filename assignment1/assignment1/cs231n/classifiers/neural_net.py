...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    softmax_output = np.exp(shift_scores)/np.sum(np.exp(shift_scores), axis = 1)[:,np.newaxis] 
    ...
    grads['W2'] =  1.0/N * H.T.dot(softmax_output) + reg* W2  
    grads['b2'] =  1.0/N * np.sum(softmax_output, axis = 0)   
    
    dH = 1.0/N * softmax_output.dot(W2.T)  
    dH_dP = (P>0) * dH  
      
    grads['W1'] = X.T.dot(dH_dP) + reg* W1  
    grads['b1'] = np.sum(dH_dP, axis = 0) 
