...
def affine_bn_relu_backward(dout, cache):
    """
    affine batchnorm relu backward
    """    
    fc_cache, bn_cache, relu_cache = cache
    dy = relu_backward(dout, relu_cache)
    da, dgamma, dbeta = batchnorm_backward_alt(dy, bn_cache)
    dx, dw, db = affine_backward(da, fc_cache)
...
def conv_bn_relu_pool_backward(dout, cache):
    ...
    ds = max_pool_backward_fast(dout, pool_cache)
    da = relu_backward(ds, relu_cache)
    dbn_out, dgamma, dbeta = spatial_batchnorm_backward(da, bn_cache)
    dx, dw, db = conv_backward_fast(dbn_out, conv_cache)
    return dx, dw, db, dgamma, dbeta
    

def conv_bn_relu_forward(x, w, b, gamma, beta, conv_param, pool_param, bn_param):
    a, conv_cache = conv_forward_fast(x, w, b, conv_param)
    bn_out, bn_cache = spatial_batchnorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(bn_out)
